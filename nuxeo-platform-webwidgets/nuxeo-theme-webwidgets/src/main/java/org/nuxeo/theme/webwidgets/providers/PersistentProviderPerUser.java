...
public class PersistentProviderPerUser extends PersistentProvider {
    ...
    public synchronized Widget createWidget(final String widgetTypeName)
        ...
        final Principal currentNuxeoPrincipal = getCurrentPrincipal();
                    ...
                    new RunCallback<WidgetEntity>() {
                        ...
                        public WidgetEntity runWith(EntityManager em) {
                            final WidgetEntity entity = new WidgetEntity(
                                    widgetTypeName);
                            entity.setScope(currentNuxeoPrincipal.getName());
                            em.persist(entity);
                            return entity;
                        }
                    }));
    ...
    public synchronized List<Widget> getWidgets(final String regionName)
        ...
        final Principal principal = getCurrentPrincipal();
        if (principal == null) {
            return new ArrayList<Widget>();
        }
        final String scope = principal.getName();

        try {
            return toDefaults(getPersistenceProvider().run(false,
                    new RunCallback<List<WidgetEntity>>() {
                        @Override
                        public List<WidgetEntity> runWith(EntityManager em) {
                            return getWidgets(em, regionName, scope);
                        }
                    }));
        } catch (ClientException e) {
    ...
    protected List<WidgetEntity> getWidgets(EntityManager em, String name, String scope) {
        Query query = em.createNamedQuery("Widget.findByScope");
        query.setParameter("region", name);
