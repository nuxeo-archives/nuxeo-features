...
public class TestVideoImporter extends SQLRepositoryTestCase {
    ...
    public void testVideoType() throws ClientException {
        ...
        session.saveDocument(docModelResult);
        
        assertEquals("/icons/video.png", docModel.getPropertyValue("common:icon"));
        assertEquals("testTitle", docModel.getPropertyValue("dc:title"));
        assertEquals("testUser", docModel.getPropertyValue("picture:credit"));
        assertEquals("testUid", docModel.getPropertyValue("uid:uid"));
        assertEquals("133", docModel.getPropertyValue("vid:duration").toString());
